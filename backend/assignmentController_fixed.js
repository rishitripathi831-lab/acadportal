...
const getAssignmentsByFaculty = (req, res) => {
  ...
  const query = `
    ...
    WHERE a.faculty_id = ?
    GROUP BY a.assignment_id, a.title, a.subject, a.deadline, a.faculty_id
...
const getSubmissionStats = (req, res) => {
  ...
  // Get submitted count, total students, and pending count directly from DB
  // Pending = students in class who have NOT submitted
  const statsQuery = `
    SELECT 
      COALESCE((SELECT COUNT(DISTINCT s.enrollment_no) FROM submissions s WHERE s.assignment_id = ?), 0) as submitted,
      COALESCE((SELECT COUNT(*) FROM students st WHERE st.department = a.branch AND st.semester = a.semester), 0) as total_students,
      COALESCE((SELECT COUNT(*) FROM students st WHERE st.department = a.branch AND st.semester = a.semester AND st.enrollment_no NOT IN (SELECT DISTINCT enrollment_no FROM submissions WHERE assignment_id = ?)), 0) as pending
    FROM assignments a
    WHERE a.assignment_id = ?
    LIMIT 1
  `;

  db.query(statsQuery, [assignment_id, assignment_id, assignment_id], (err, results) => {
    if (err) {
    ...
    console.log('Submission stats: assignment_id=%d, submitted=%d, total=%d, pending=%d', assignment_id, submitted, total_students, pending);
